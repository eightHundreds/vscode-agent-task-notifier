...
MESSAGE="Claude finished current turn"
NOW_MS="$(( $(date +%s) * 1000 ))"
...
EVENT_JSON="$(
  jq -cn \
    ...
    --arg taskId "${TASK_ID}" \
    --arg dedupeKey "claude:stop:${SESSION_ID}:${TASK_ID}" \
...
if [[ -w /dev/tty ]]; then
  if { printf '%s' "${OSC}" > /dev/tty; } 2>/dev/null; then
    log_debug "emitted stop via /dev/tty session=${SESSION_ID} task=${TASK_ID}"
    exit 0
  fi
...
if write_to_parent_tty "${OSC}"; then
  log_debug "emitted stop via parent-tty session=${SESSION_ID} task=${TASK_ID}"
  exit 0
fi

printf '%s' "${OSC}"
printf '%s' "${OSC}" >&2
log_debug "emitted stop via stdout+stderr session=${SESSION_ID} task=${TASK_ID}"
